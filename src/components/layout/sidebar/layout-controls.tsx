...
import { usePreferencesStore } from "@/app/stores/preferences/preferences-provider";
import type {
  SidebarVariant,
  SidebarCollapsible,
  ContentLayout,
...
export function LayoutControls(props: LayoutControlsProps) {
  ...
  const handleValueChange = async (key: string, value: LayoutValue) => {
    ...
    if (
      key === "theme_preset" &&
      THEME_PRESET_OPTIONS.some((option) => option.value === value)
    ) {
